...
const OLD_DB_ID = '93490bfb-ac43-49be-a24d-9e3e5a225991';
...
async function main() {
    ...
    await notionRequest('PATCH', `/v1/pages/${OLD_DB_ID}`, { archived: true });
    console.log('✅ 旧数据库已删除\n');
    ...
    if (status !== 200) {
        console.error('❌ 创建失败:', db.message || db);
        process.exit(1);
    }
    ...
    // 3. 更新配置
    console.log('\n[3/3] 更新配置...');
    const configContent = `// 竞彩Notion配置（自动生成）\nmodule.exports = {\n  DATABASE_ID: '${newDbId}'\n};\n`;
    fs.writeFileSync('jingcai/config.js', configContent, 'utf8');
    console.log('✅ 配置已更新: jingcai/config.js');
    ...
    console.log('✅ 同步脚本已更新: jingcai/sync_notion.js');
