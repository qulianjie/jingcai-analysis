...
# Step 2: Build date -> {match_num -> teams} from ALL match directories
date_num_teams = {}
for date in sorted(os.listdir(TASKS_DIR)):
    ...
    # Old format: matchNNN__teamA_teamB (directly in date folder)
...
# Step 3: Build team -> league map from ALL dates that have BOTH league and team data
team_to_league = {}
...
# Step 4: For unknown entries, try to match by team name
unknown_dates = [
    ...
    for item in date_data.get('feedback', []):
        ...
        if not league or league == '未知' or league.strip() == '':
            ...
            if len(common) == 1:
                ...
                # Try partial match: if one team is known and the other is not, 
                # check if it's a cross-league match (like Champions League)
                if home_leagues and not away_leagues:
                    # Home team is in known leagues, away team not in map
                    # This could be a cup match
                    auto_tagged.append({
                        'date': date_str,
                        'num': mn,
                        'teams': teams_str,
                        'league': f'待确认(主:{", ".join(sorted(home_leagues))})',
                        'score': item.get('score', ''),
                        'partial': True,
                    })
                elif away_leagues and not home_leagues:
                    auto_tagged.append({
                        'date': date_str,
                        'num': mn,
                        'teams': teams_str,
                        'league': f'待确认(客:{", ".join(sorted(away_leagues))})',
                        'score': item.get('score', ''),
                        'partial': True,
                    })
                else:
...
# Show auto-tagged
print(f'\n=== 自动标记 (全部) ===')
for t in auto_tagged:
    tag = ' ⚠️部分' if t.get('partial') else ''
    print(f'  [{t["date"]}] {t["num"]} {t["teams"]} → {t["league"]} ({t["score"]}){tag}')
