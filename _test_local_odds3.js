...
function extractOddsFromLocalFiles(dateStr) {
    ...
    for (const file of files) {
        try {
            ...
            // 列:    公司     初胜  初平  初负  即胜  即平  即负  变化
            const lines = content.split('\n');
            for (const line of lines) {
                if (line.includes('竞彩官方') && line.includes('|')) {
                    const parts = line.split('|').map(p => p.trim()).filter(p => p);
                    // 检查是否是欧赔行（有7个字段: 公司+3初盘+3即时+变化=8个，但filter后是8个）
                    if (parts.length >= 8 && parts[1] !== '-1') {  // 排除让球行
                        const win = parseFloat(parts[4]);
                        const draw = parseFloat(parts[5]);
                        const loss = parseFloat(parts[6]);
                        if (!isNaN(win) && !isNaN(draw) && !isNaN(loss)) {
                            odds[matchNum] = { win, draw, loss };
                            break;
                        }
                    }
                }
