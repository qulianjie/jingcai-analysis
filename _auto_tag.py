# -*- coding: utf-8 -*-
import json, os, sys, glob, re
sys.stdout.reconfigure(encoding='utf-8')

TASKS_DIR = r'C:\Users\lianjie\.openclaw\workspace\jingcai\tasks'

# 1. Build team -> league mapping from entries that HAVE league data
fb = json.load(open(r'C:\Users\lianjie\.openclaw\workspace\jingcai\learnings\feedback.json', encoding='utf-8'))

# First pass: build team->league from known entries
# We need to parse match directories to get team names
team_to_league = {}  # team_name -> set of leagues

# Scan all dates for match directories with meta.json
for date in sorted(os.listdir(TASKS_DIR)):
    date_dir = os.path.join(TASKS_DIR, date)
    if not os.path.isdir(date_dir):
        continue
    
    # Try data/match*/meta.json
    data_dir = os.path.join(date_dir, 'data')
    if os.path.isdir(data_dir):
        for d in os.listdir(data_dir):
            if not d.startswith('match'):
                continue
            meta_file = os.path.join(data_dir, d, 'meta.json')
            if os.path.exists(meta_file):
                try:
                    meta = json.load(open(meta_file, encoding='utf-8'))
                    league = meta.get('league', '')
                    home = meta.get('home_team', '')
                    away = meta.get('away_team', '')
                    if league and league != '未知' and home and away:
                        if home not in team_to_league:
                            team_to_league[home] = set()
                        team_to_league[home].add(league)
                        if away not in team_to_league:
                            team_to_league[away] = set()
                        team_to_league[away].add(league)
                except:
                    pass
    
    # Try match*/meta.json (old format)
    for d in os.listdir(date_dir):
        if not d.startswith('match'):
            continue
        meta_file = os.path.join(date_dir, d, 'meta.json')
        if os.path.exists(meta_file):
            try:
                meta = json.load(open(meta_file, encoding='utf-8'))
                league = meta.get('league', '')
                home = meta.get('home_team', '')
                away = meta.get('away_team', '')
                if league and league != '未知' and home and away:
                    if home not in team_to_league:
                        team_to_league[home] = set()
                    team_to_league[home].add(league)
                    if away not in team_to_league:
                        team_to_league[away] = set()
                    team_to_league[away].add(league)
            except:
                pass

print(f'球队→联赛映射: {len(team_to_league)}支球队')

# 2. For unknown entries, try to auto-tag
unknown_dates = [
    '2026-04-03', '2026-04-04', '2026-04-05', '2026-04-06', '2026-04-07',
    '2026-04-08', '2026-04-09', '2026-04-10', '2026-04-11', '2026-04-12',
    '2026-04-13', '2026-04-14', '2026-04-15', '2026-04-16', '2026-04-17',
    '2026-04-18', '2026-04-19', '2026-04-20', '2026-04-21', '2026-04-22',
    '2026-04-29',
]

# Build match_num -> teams
match_teams = {}
for date in unknown_dates:
    date_dir = os.path.join(TASKS_DIR, date)
    if not os.path.isdir(date_dir):
        continue
    
    # From md reports
    for f in glob.glob(os.path.join(date_dir, '周*.md')):
        m = re.match(r'周[一二三四五六日](\d+)[_~](.+)\.md', os.path.basename(f))
        if m:
            num = m.group(1).strip()
            teams = m.group(2).strip()
            match_teams[num] = teams
    
    # From match directories
    for d in os.listdir(date_dir):
        if not d.startswith('match'):
            continue
        meta_file = os.path.join(date_dir, d, 'meta.json')
        if os.path.exists(meta_file):
            try:
                meta = json.load(open(meta_file, encoding='utf-8'))
                home = meta.get('home_team', '')
                away = meta.get('away_team', '')
                if home and away:
                    num_match = re.match(r'match(\d+)', d)
                    if num_match:
                        num = num_match.group(1)
                        match_teams[num] = f'{home}vs{away}'
            except:
                pass

# 3. Try to auto-tag unknown entries
auto_tagged = []
cannot_tag = []

for date_str in unknown_dates:
    date_data = fb.get('dates', {}).get(date_str, {})
    if not date_data:
        continue
    
    for item in date_data.get('feedback', []):
        league = item.get('league', '')
        if not league or league == '未知' or league.strip() == '':
            mn = item.get('match_num', '')
            teams_str = match_teams.get(mn, '')
            
            if not teams_str or 'vs' not in teams_str:
                cannot_tag.append({
                    'date': date_str,
                    'num': mn,
                    'teams': teams_str or '???',
                    'score': item.get('score', ''),
                })
                continue
            
            # Split teams
            parts = teams_str.split('vs')
            if len(parts) != 2:
                cannot_tag.append({
                    'date': date_str,
                    'num': mn,
                    'teams': teams_str,
                    'score': item.get('score', ''),
                })
                continue
            
            home_team = parts[0].strip()
            away_team = parts[1].strip()
            
            # Check if both teams are in the same league
            home_leagues = team_to_league.get(home_team, set())
            away_leagues = team_to_league.get(away_team, set())
            
            common = home_leagues & away_leagues
            
            if len(common) == 1:
                league_name = list(common)[0]
                auto_tagged.append({
                    'date': date_str,
                    'num': mn,
                    'teams': teams_str,
                    'league': league_name,
                    'score': item.get('score', ''),
                })
            elif len(common) > 1:
                auto_tagged.append({
                    'date': date_str,
                    'num': mn,
                    'teams': teams_str,
                    'league': ', '.join(common),
                    'score': item.get('score', ''),
                })
            else:
                cannot_tag.append({
                    'date': date_str,
                    'num': mn,
                    'teams': teams_str,
                    'home_leagues': ', '.join(home_leagues) if home_leagues else '无记录',
                    'away_leagues': ', '.join(away_leagues) if away_leagues else '无记录',
                    'score': item.get('score', ''),
                })

print(f'\n自动标记: {len(auto_tagged)}场')
print(f'无法标记: {len(cannot_tag)}场')

# Show auto-tagged
print('\n=== 自动标记 ===')
for t in auto_tagged[:10]:
    print(f'  [{t["date"]}] {t["num"]} {t["teams"]} → {t["league"]} ({t["score"]})')
if len(auto_tagged) > 10:
    print(f'  ... 还有{len(auto_tagged)-10}场')

# Show cannot-tagged
print(f'\n=== 需要你确认 ({len(cannot_tag)}场) ===')
by_date = {}
for c in cannot_tag:
    d = c['date']
    if d not in by_date:
        by_date[d] = []
    by_date[d].append(c)

for d in sorted(by_date.keys()):
    items = by_date[d]
    print(f'\n[{d}] {len(items)}场:')
    for c in items:
        hl = c.get('home_leagues', '')
        al = c.get('away_leagues', '')
        if hl or al:
            print(f'  {c["num"]}  {c["teams"]}  {c["score"]}  |  主:{hl}  客:{al}')
        else:
            print(f'  {c["num"]}  {c["teams"]}  {c["score"]}')
