...
for d in sorted(os.listdir(BASE)):
    ...
    for f in sorted(os.listdir(dp)):
        ...
        try:
            ...
            matchnum_map[d] = matchnum_map.get(d, []) + [matchnum]
            
            # 提取比分: "实际比分: 2:1" 或 "竞彩: 310 → 胜 (2:0)"
            score_match = re.search(r'(?:实际比分|比分)[:：\s]*(\d+)[：:](\d+)', content)
            if not score_match:
                score_match = re.search(r'(?:竞彩|让球)[^\n]*→\s*[^\n(]*(?:\((\d+)[：:](\d+)\))', content)
...
print(f'从报告提取比分: {len(score_map)}场')
