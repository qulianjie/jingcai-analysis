...
def run_match_pipeline(match, date_str, match_index):
    ...
    if not home_id or not macau_line:
        ...
        try:
            script_path = os.path.join(SCRIPT_DIR, 'extract_team_info.py')
    ...
    steps = [
        ...
        (2, '同赔(2+3+5+7)', 'step2357_runner.py', [fid, league,
            os.path.join(match_dir, 'group01_europe', 'step02_jingcai_same.md'),
            os.path.join(match_dir, 'group01_europe', 'step03_interwetten_same.md'),
            os.path.join(match_dir, 'group02_handicap', 'step05_handicap_same.md'),
            os.path.join(match_dir, 'group03_asian', 'step07_macau_same.md')]),
...
def main():
    ...
    run_script('step25_zhuangjia.py', [date_str], timeout=300)
    
    # Re-generate all reports with step 25 data
