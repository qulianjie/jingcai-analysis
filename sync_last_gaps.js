...
const DATES = ['2026-05-03','2026-05-04','2026-05-10','2026-05-12'];
...
async function main() {
    ...
    for (const dateStr of DATES) {
        ...
        for (const p of existing) {
            ...
            if (mnMatch) existingMns.add(mnMatch[1]);
        }
        
        const missing = Object.entries(matchMap).filter(([mn]) => !existingMns.has(mn));
        console.log(dateStr + ': 缺 ' + missing.length + ' 场');
        
        for (const [mn, matchDir] of missing) {
