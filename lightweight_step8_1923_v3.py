...
# 联赛ID映射
LEAGUE_ID_MAP = {
    '韩职': '19554', '芬超': '19510', '荷乙': '19502',
    '沙特': '19506', '欧冠': '19538', '解放者杯': '19546',
}

# 硬编码球队ID（从联赛页提取不可靠，直接硬编码）
# 韩职12队
K1_TEAM_IDS = ['1603','1606','1608','1609','1611','1612','1615','3410','4544','5739','6477','7075']
# 芬超12队（2026赛季）
FIN_TEAM_IDS = ['3446','3447','3448','3449','3450','3451','3452','3453','3454','3455','3456','3457']
# 荷乙20队（Eerste Divisie）
# 实际是荷兰乙级联赛，需要正确球队
NED_TEAM_IDS = []  # 先空，后面从联赛页尝试
# 沙特18队
SAU_TEAM_IDS = []
# 欧冠球队（从小组赛/淘汰赛阶段）
UCL_TEAM_IDS = []
...
for league, league_matches in matches_by_league.items():
    ...
    # 优先用硬编码球队ID
    team_ids = TEAM_IDS_BY_LEAGUE.get(league, [])
    ...
    if not team_ids:
        ...
        if league_id:
            ...
            print(f"  从联赛页获取球队: {len(team_ids)} 支")
    
    if not team_ids:
        print(f"  ⚠️ 无法获取球队列表，跳过")
        continue
    
    print(f"  球队ID: {team_ids}")
    ...
    for h, c in sorted(hcp_dist.items(), key=lambda x: -x[1]):
        print(f"    {h}: {c}场")
