...
用法1: python step235_runner.py <fid> <league> <step2_out> <step3_out> <step5_out>
...
if len(sys.argv) > 1 and os.path.isdir(sys.argv[1]):
    ...
    if os.path.exists(meta_path):
        ...
        FID = '1199680'
        LEAGUE = '意甲'
    STEP2_OUT = os.path.join(MATCH_DIR, 'group01_europe', 'step2_jingcai_same.txt')
    STEP3_OUT = os.path.join(MATCH_DIR, 'group01_europe', 'step3_interwetten_same.txt')
    STEP5_OUT = os.path.join(MATCH_DIR, 'group02_handicap', 'step5_handicap_same.txt')
else:
...
def fetch_ouzi_odds(fid, company_row='1'):
...
def fetch_iw_odds(fid):
    """Fetch Interwetten odds (row 6) from 500.com ouzhi page"""
    return fetch_ouzi_odds(fid, company_row='6')
...
def write_step(title, bench_odds, data, fetch_func, is_rangqiu=False, out=sys.stdout):
    ...
    # 本场比赛的初盘赔率 = 同赔查询的基准赔率
    # 所有历史比赛的初盘应该等于本场比赛初盘（因为是用这个赔率查出来的）
    bench_init = bench_odds['init']
    bench_live = bench_odds['live']
