# -*- coding: utf-8 -*-
"""补完match24 + 补跑剩余5场：022, 023, 026, 029, 030"""
...
# First: complete match24 (021) - missing step9-18 and step24
print('=== Complete match24 (021) ===')
match24_dirs = [d for d in os.listdir(DATA_DIR) if d.startswith('match24_') and os.path.isdir(os.path.join(DATA_DIR, d))]
if match24_dirs:
    match24_dir = os.path.join(DATA_DIR, match24_dirs[0])
    print(f'Dir: {match24_dirs[0]}')
    
    # Run step9-18
    print('  -> step918_extractor.py...')
    try:
        r = subprocess.run([sys.executable, os.path.join(SCRIPT_DIR, 'step918_extractor.py'), match24_dir], 
    ...
    try:
        r = subprocess.run([sys.executable, os.path.join(SCRIPT_DIR, 'step24_extractor.py'), match24_dir], 
                          timeout=1800, encoding='utf-8', errors='replace')
    ...
    try:
        r = subprocess.run(
            [sys.executable, os.path.join(SCRIPT_DIR, 'final_report_generator.py'), match24_dir],
