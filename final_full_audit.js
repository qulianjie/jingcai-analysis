...
async function main() {
    let totalGames = 0, totalZeroRq = 0, totalZeroMa = 0, totalPredicted = 0, totalFeedback = 0;
    ...
    for (let d = 1; d <= 12; d++) {
        ...
        let zeroRq = 0, zeroMa = 0, predicted = 0, feedback = 0;
        const zeroRqNames = [];
        ...
        for (const p of games) {
            ...
            if (!rq || rq === 0) { zeroRq++; zeroRqNames.push(p.properties.Name?.title?.[0]?.plain_text || ''); }
            if (!ma) zeroMa++;
            if (pred) predicted++;
            if (score) feedback++;
        }
        
        // Also count local matches
        const dateDir = path.join(tasksDir, dateStr, 'data');
        let localCount = 0;
        if (dateDir && fs.existsSync(dateDir)) {
            localCount = fs.readdirSync(dateDir).filter(d => d.startsWith('match')).length;
        }
        ...
        console.log(dateStr + ': Notion=' + games.length + ', 本地=' + localCount + ', 让球0=' + zeroRq + ', 澳门空白=' + zeroMa + ', 有预测=' + predicted + ', 有比分=' + feedback);
    }
    
    console.log('\n总计: Notion=' + totalGames + ', 让球0=' + totalZeroRq + ', 澳门空白=' + totalZeroMa + ', 有预测=' + totalPredicted + ', 有比分=' + totalFeedback);
}
