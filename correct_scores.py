...
# 从搜索结果确认的实际比分（来自vipc.cn）
# 之前 reapply_scores4.py 解析有误，以下为依据搜索结果修正的比分
CORRECT_SCORES = {
    '周日001': '1:0', '周日002': '0:1', '周日003': '1:0', '周日004': '0:2',
    '周日005': '0:1', '周日006': '2:0', '周日007': '1:1', '周日008': '2:0',
    '周日009': '2:2', '周日010': '2:2', '周日011': '1:1', '周日012': '3:0',  # 修正: 009/010/012
...
for mn in sorted(CORRECT_SCORES.keys()):
    ...
    if mn in WRONG_SCORES:
        print(f'  {mark} {mn}: {score} {actual_r} | 竞彩:{pred}→{pred_r}')
