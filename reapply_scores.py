...
for mn in sorted(SCORES.keys()):
    ...
    exp = SCORES[mn]
    mark = 'OK' if cur == exp else f'WRONG(now:{cur})'
    print(f'  {mn}: {exp} <- {mark}')

# 4) Now re-apply all 30
print('\n=== 重新写入30场比分 ===')
for mn in sorted(SCORES.keys()):
    page = page_map.get(mn)
    if not page:
    ...
    if r.status_code == 200:
        ...
        print(f'  {mn}: FAIL {r.status_code} {r.text[:200]}')
...
for mn in sorted(SCORES.keys()):
    ...
    if cur == exp:
        ...
        print(f'  {mn}: expected={exp} got={cur} !!!')
