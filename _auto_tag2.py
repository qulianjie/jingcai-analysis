...
# Step 1: For each date, build match_num -> league from feedback entries that HAVE league
fb = json.load(open(r'C:\Users\lianjie\.openclaw\workspace\jingcai\learnings\feedback.json', encoding='utf-8'))

# date -> {match_num -> league}
date_num_league = {}
...
print(f'有联赛数据的日期: {len(date_num_league)}个')

# Step 2: For each date, build match_num -> teams from match directories
# Old format: date/match001__teamA_teamB/meta.json
# New format: date/data/match1_A_teamA__teamB/meta.json
...
for date in sorted(os.listdir(TASKS_DIR)):
    ...
    # Old format: match001__teamA_teamB
    for d in os.listdir(date_dir):
        ...
        if os.path.exists(meta_file):
            try:
                ...
                # Extract number from "match001__..." -> "001"
...
# Step 3: Build team -> league map (date-specific)
# For dates that have BOTH league AND team data, extract the mapping
team_to_league = {}  # team -> set of leagues
...
# Step 4: Now auto-tag unknown entries
unknown_dates = [
