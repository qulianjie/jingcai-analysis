...
for t, n in needs:
    m = match_map[n]
    print(f'  {t}: {n} - {m.get("home","")} vs {m.get("away","")}')
...
for typ, num in needs:
    ...
    if typ == 'CREATE':
        ...
        with open(os.path.join(match_dir, 'meta.json'), 'w', encoding='utf-8') as f:
            json.dump({
                'matchnum': m.get('matchnum',''), 'match': '{} {} vs {}'.format(m.get('matchnum',''), home, away),
                'fid': m.get('fid',''), 'league': m.get('league',''),
                'home': home, 'away': away, 'date': '2026-05-16', 'status': 'in_progress',
    ...
    print(f'[{num}] {os.path.basename(match_dir)}')
    ...
    for script in scripts:
        p = subprocess.Popen([sys.executable, os.path.join(SCRIPT_DIR, script), match_dir],
...
print(f'Done. {len(seen)} unique reports')
